...
export class PaymentsService {
  ...
  async handleWebhook(rawBody: Buffer, signature: string) {
    ...
    switch (event.type) {
      case "payment_intent.succeeded": {
        ...
        // === NEU: TravelOrder-Branch ===
        if (
          (pi.metadata?.kind || "") === "travel_order" &&
          pi.metadata?.travel_order_id
        ) {
          const travelOrderId = pi.metadata.travel_order_id;

          // 1) TravelOrder holen (idempotent)
          const tro = await this.prisma.travelOrder.findUnique({
            ...
            select: {
              ...
              flightOrderId: true,
              groundTransportOrderId: true,
            },
          });
          if (!tro) {
            this.logger.warn(
              `travel_order ${travelOrderId} not found for PI ${pi.id}`
            );
            break;
          }
          if (tro.status === "paid" || tro.status === "fulfilled") {
            this.logger.debug(
              `travel_order ${travelOrderId} already ${tro.status}`
            );
            break;
          }

          // 2) Duffel bezahlen, wenn HOLD (idempotent via pi.id)
          if (tro.flightOrderId) {
            const fo = await this.prisma.order.findUnique({
              where: { id: tro.flightOrderId },
              select: { duffelId: true, amount: true, currency: true },
            });
            const duffelOrderId = fo?.duffelId ?? null;

            if (duffelOrderId) {
              try {
                const dOrder = await this.duffel.getOrder(duffelOrderId);
                const awaiting =
                  dOrder?.payment_status?.awaiting_payment === true ||
                  dOrder?.type === "hold";
                if (awaiting) {
                  ...
                  // Instant-Fall: nur PI referenzieren
                  await this.prisma.order
                    .updateMany({
                      where: { duffelId: duffelOrderId },
                      data: {
                ...
                // eTicket Poll idempotent
                await this.eticketQueue
                  .add(
                    "poll",
                    { orderId: duffelOrderId, attempt: 1 },
                    {
                      jobId: `poll:${duffelOrderId}`,
                      delay: 3000,
                ...
                this.logger.error(
                  `Duffel settlement (travel_order) failed: ${e}`
                );
                // kein Throw -> Stripe soll Webhook nicht ewig retrien
              }
            }
          }

          // 3) Ground confirm (idempotent/leichtgewichtig hier direkt)
          if (tro.groundTransportOrderId) {
            await this.prisma.groundTransportOrder
              .update({
                where: { id: tro.groundTransportOrderId },
                data: { status: "confirmed", confirmedAt: new Date() as any },
              })
              .catch((e) => this.logger.warn(`ground confirm failed: ${e}`));
          }

          // 4) TravelOrder als bezahlt markieren
          await this.prisma.travelOrder
            .update({
              where: { id: travelOrderId },
              data: {
                status: "paid",
            ...
            .catch(() => {});
          break; // 👉 TravelOrder-Branch beendet
        }
  ...
  // === TravelOrder: automatic-capture PaymentIntent (Flight+Ground Gesamtbetrag) ===
  async createIntentForTravelOrder(travelOrderId: string, userId?: string) {
    // TravelOrder mit total & currency laden
    const tro = await this.prisma.travelOrder.findUnique({
      where: { id: travelOrderId },
      select: {
        id: true,
        ...
        currency: true,
        paymentIntentId: true,
        flightOrderId: true, // interne Order.id (zeigt auf deine Order mit duffelId)
        groundTransportOrderId: true,
      },
    });
    ...
    if (!tro) {
      return this.resultFail(
        "travel_order_not_found",
        `TravelOrder ${travelOrderId} not found`
      );
    }
    if (tro.status === "paid" || tro.status === "fulfilled") {
      return this.resultFail(
        "already_paid",
        "TravelOrder is already paid/fulfilled",
        { travel_order_id: tro.id }
      );
    }

    const currency = (tro.currency || "EUR").toUpperCase();
    const amountStr = String(tro.totalAmount);
    let amountMinor: number;
    try {
      amountMinor = toMinorUnits(amountStr, currency);
    } catch {
      return this.resultFail(
        "invalid_totals",
        "Invalid TravelOrder amount/currency",
        {
          travel_order_id: tro.id,
    ...
    // Duffel-Order-ID (nur für Metadaten/Orchestrierung)
    let duffelOrderId: string | null = null;
    if (tro.flightOrderId) {
      const fo = await this.prisma.order.findUnique({
        where: { id: tro.flightOrderId },
        select: { duffelId: true },
      });
      duffelOrderId = fo?.duffelId ?? null;
    }

    const idemKey = crypto
      .createHash("sha256")
      .update(`travel:${tro.id}:${amountStr}:${currency}`)
      .digest("hex");
    ...
    try {
      const intent = await this.stripe.paymentIntents.create(
        {
          ...
          metadata: {
            kind: "travel_order",
            travel_order_id: tro.id,
            ...(duffelOrderId ? { duffel_order_id: duffelOrderId } : {}),
            ...(userId ? { user_id: userId } : {}),
          },
        },
        { idempotencyKey: idemKey }
      ...
      // PI an TravelOrder hängen (idempotent)
      await this.prisma.travelOrder
        .update({
          where: { id: tro.id },
